...
/*   void listenForFoods2(String idRestaurant, String categoryId) async {
    final Stream<Food> stream =
        await getFoodsOfRestaurant2(idRestaurant, categoryId);
    stream.listen((Food _food) {
      if (foods.isEmpty) {
        setState(() => foods = [_food]);
      } else {
        setState(() => foods.add(_food));
      }
    }, onError: (a) {
  ...
  Future<void> listenForFoods2(String restaurantId, String categoryId) async {
    final Stream<Food> stream =
        await getFoodsOfRestaurant2(restaurantId, categoryId);
    stream.listen((Food _food) {
      setState(() {
        foods.add(_food);
      });
