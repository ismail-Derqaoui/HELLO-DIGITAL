...
Future<Stream<Food>> getFoodsOfRestaurant2(
  ...
  if (categoryId != '0') {
    query['search'] += ' AND category_id:$categoryId';
    query['searchFields'] += ';category_id:=';
  }
  ...
  try {
    ...
    return streamedRest.stream
        ...
        .map((data) {
      return Food.fromJSON(data);
    }).where((food) => categoryId == '0' || food.category.id == categoryId);
  } catch (e) {
    print(CustomTrace(StackTrace.current, message: uri.toString()).toString());
    return new Stream.value(new Food.fromJSON({}));
  }
